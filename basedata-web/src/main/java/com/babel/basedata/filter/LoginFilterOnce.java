 ...
 *
 */
public class LoginFilterOnce extends OncePerRequestFilter {
...
//	private static String RETRY_FUNC_CODE_LOGINFILTER="f_loginFilter";
	...
	private boolean isFilter(String reqUri){
		...
		if ("/".equals(reqUri) || "/code.do".equals(reqUri)) {
			isFilter = true;
			return isFilter;
		}
...
//		String sessionId=(String)req.getAttribute(SHIRO_SESSION_ID);
		...
		String ip=AppContext.getIpAddr(req);
...
//		}
		...
		if(!isFilter){
			...
			if(userInfo==null){
				logger.info("----userInfo=null sessionId="+sessionId);
			}
			if(userInfo!=null){
	...
	//			logger.info("---filter-"+reqUri+" sessionId="+sessionId+" userId="+userInfo.getUserId()+" roleId="+userInfo.getRoleIdStr()+" isPermit="+isPermit);
				...
				if(!isPermit){
//					logger.warn("---filter-"+reqUri+" userId="+userInfo.getUserId()+" roleId="+userInfo.getRoleIdStr()+" isPermit="+isPermit);
					userInfo.setFilterCount(userInfo.getFilterCount()+1);
					res.sendError(HttpStatus.DESTINATION_LOCKED.value(), "permission deny");
					return;
				}
			}
		...
		if(!isFilter && this.retryRuleCheckService!=null){
			...
			logger.info("----servletInfo isCheckUrl="+isCheckUrl);
			if(isCheckUrl){
		...
		System.out.println("--------sessionId="+req.getRequestedSessionId()+"/"+req.getAttribute(SHIRO_SESSION_ID));
