$('#mainWindow').css('display','block');//用于避免chrome下页面刚打开时，画面会有一小会的格式不好的问题
		...
		$('#query_funcCode').keydown(listenerName);
	
        var url;
        function onAdd(){
            $('#dlg').dialog('open').dialog('setTitle','New Model');
            $('#fm').form('clear');
            $('#model_status').combobox('select', '1');
            $('#model_showType').combobox('select', '0');
            //$("#model_remark").val('test');
        }
        ...
        function onDestroy(){
            ...
            if (row){
                $.messager.confirm('Confirm','Are you sure you want to destroy this data?',function(r){
                    if (r){
                        $.post('/basedata/model/delete',{id:row.cid},function(result){
                            if (result.success){
                            	doSearchReload();    // reload the user data
                            } else {
