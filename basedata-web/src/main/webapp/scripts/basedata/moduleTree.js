$('#mainWindow').css('display','block');//用于避免chrome下页面刚打开时，画面会有一小会的格式不好的问题
        ...
        function doSearchTree(){
        	$.post('/basedata/module/getModuleByName',{name: $("#query_name").val()}
    			,function(result){
                    if (result.success){
		...
		function onDestroy(){
            if (selectNode){
                $.messager.confirm('Confirm','Are you sure you want to destroy this data?',function(r){
                    if (r){
                        $.post('/basedata/module/delete',{id:selectNode.id},function(result){
                            if (result.success){
		         ...
		         type : "post",  
		          ...
		          success : function(result){  
		            	if (result.success){
                            ...
                            //console.log('---onBeforeDrop--false---');
                            isSuccess= false;
                        }
		          }  
		     }); 
			console.log('---onBeforeDrop--'+isSuccess);
			return isSuccess;
		}
		/**
...
//			var targetId = $('#menuTree').tree('getNode', target).id;
//			console.log('----id='+source.id+' oldParentId='+source.parentId+' newParentId='+targetId);
//			$.post('/basedata/module/updateRel',{cid:source.id, oldParentId:source.parentId, newParentId:targetId},function(result){
//                            if (result.success){
//                            	//$('#menuTree').tree('remove', selectNode.target);
//                            } else {
//                                $.messager.show({    // show error message
...
//                        },'json');
		...
		function onBeforeExpand(node, param){
			$('#menuTree').tree('options').url = "/basedata/module/findModuleByParentId?moduleId="+node.id;     
		}
