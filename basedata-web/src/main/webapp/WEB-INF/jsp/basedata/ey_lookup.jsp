...
<body>
 	...
 	<div id="mainWindow" class="easyui-layout" data-options="fit:true" style="display:none">
		...
		<div data-options="region:'north', title:'数据字典-条件'"
			...
			<form id="search-form" >
	            ...
	            <div class="fitem">
	                <label>名称:</label>
	                <input name="nameCn" class="input_query" id="query_nameCn">
	            </div>
	            <div class="fitem">
	                <label>状态:</label>
		        ...
		        <thead>
		            <tr >
		            	...
		            	*/ %>
		                ...
		                <th field="nameCn" width="50" sortable="true">中文名</th>
		                <th field="nameEn" width="50"  hidden="true">英文名</th>
    ...
    <div id="dlg" class="easyui-dialog" style="width:300px;height:300px;padding:5px 10px;display:none"
        ...
        <fieldset>
	        ...
	        <form id="fm" method="post" novalidate>
	            ...
	            <div class="fitem">
	                <label>中文名:</label>
	                <input name="nameCn" class="easyui-validatebox" required="true">
	            </div>
	            <div class="fitem">
	                <label>英文名:</label>
	            ...
	            <div class="fitem">
	                <label>状态:</label>
	                <input class="easyui-combobox" name="status" id="lookup_status" required="true" data-options="
									valueField: 'id',
									textField: 'name',
	            ...
	            <div class="fitem">
	                <label>是否可修改:</label>
	                <input class="easyui-combobox" name="canModify" id="lookup_canModify" required="true" data-options="
									valueField: 'id',
									textField: 'name',
									//multiple:true, //多选增加此项
	            ...
	            <div class="fitem">
	                <label>备注:</label>
	                <textarea name="remark" id="lookup_remark" style="height:60px;width:200px"></textarea>
	            </div>
	        </form>
        </fieldset>
        <div id="dlg-buttons">
    ...
    <script type="text/javascript">
		...
		function onClick(rowIndex, rowData){
			if(parent){
				console.log('---onclick---');
				parent.iframeCenterSearch({lookupId:rowData.cid,canModify:rowData.canModify});
			}
        }
        ...
        var url;
        function onAdd(){
            $('#dlg').dialog('open').dialog('setTitle','New Lookup');
            $('#fm').form('clear');
            $('#lookup_status').combobox('select', '1');
            //$("#lookup_remark").val('test');
        }
        ...
        function onEdit(){
            ...
            if (row){
            	...
            	//alert(obj.'lookup.empCode');
                $('#dlg').dialog('open').dialog('setTitle','Edit Lookup');
                $('#fm').form('clear');
                $('#fm').form('load',obj);
            }
        ...
        function onDestroy(){
            ...
            if (row){
                $.messager.confirm('Confirm','Are you sure you want to destroy this data?',function(r){
                    if (r){
                        $.post('/basedata/lookup/delete',{id:row.cid},function(result){
                            if (result.success){
                            	doSearchReload();    // reload the user data
                            } else {
