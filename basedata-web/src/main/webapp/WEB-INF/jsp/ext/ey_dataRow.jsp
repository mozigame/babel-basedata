...
<head>
	<meta charset="UTF-8">
    <title>行形扩展表数据table_type_id,ref_data_type组合后可相当于子表table_type_</title>
...
<body>
 	...
 	<div class="easyui-layout" data-options="fit:true">
    	<!-- 查询条件  -->
		<div data-options="region:'north', title:'行形扩展表数据table_type_id,ref_data_type组合后可相当于子表table_type_-条件'"
		    ...
		    <table id="dg" class="easyui-datagrid" data-options="onDblClickRow:onDblClick, onClickRow:onClick, onLoadSuccess:function(){$('.datagrid-btable').find('div.datagrid-cell').css('text-align','left');}"
		        ...
		        <thead>
		            <tr >
		            	...
		            	*/ %>
						<th align="center" field="projectId" width="50" sortable="true">产品id</th>
						...
						<th align="center" field="data" width="50" sortable="true">数据</th>
						<th align="center" field="dataVar" width="50" sortable="true">字符数据</th>
						<th align="center" field="dataDate" width="50" sortable="true">日期数据</th>
						<th align="center" field="dataDatetime" width="50" sortable="true">时间数据</th>
						<th align="center" field="dataNum" width="50" sortable="true">数字数据</th>
						<th align="center" field="dataFloat" width="50" sortable="true">浮点数，数据</th>
		                <%/*  */ %>
    ...
    <div id="dlg" class="easyui-dialog" style="width:400px;height:500px;padding:5px 10px;display:none;"
        ...
        <fieldset>
			<legend>
				行形扩展表数据table_type_id,ref_data_type组合后可相当于子表table_type_
			</legend>   
	        <form id="fm" method="post" novalidate>
				...
				<div class="fitem">
	                <label>字符数据:</label>
	                <input name="dataVar" class="easyui-validatebox" required="true">
	            </div>	
				<div class="fitem">
	                <label>日期数据:</label>
	                <input name="dataDate" class="easyui-datebox" required="true">
	            </div>	
				<div class="fitem">
	                <label>时间数据:</label>
	                <input name="dataDatetime" class="easyui-datebox" required="true">
	            </div>	
				<div class="fitem">
	                <label>数字数据:</label>
	                <input name="dataNum" class="easyui-validatebox" required="true">
	            </div>	
				<div class="fitem">
	                <label>浮点数，数据:</label>
	                <input name="dataFloat" class="easyui-validatebox" required="true">
	            </div>	
				<div class="fitem">
	                <label>备注:</label>
	            ...
	            <div class="fitem">
	                <label>状态:</label>
	                <input class="easyui-combobox" name="status" id="dataRow_status" required="true" data-options="panelHeight:'auto',
									valueField: 'id',
									textField: 'name',
	            ...
	            <div class="fitem">
	                <label>备注:</label>
	                <textarea name="remark" id="dataRow_remark" style="height:60px;width:200px"></textarea>
	            </div>
	            */ %>
	        </form>
        </fieldset>
    ...
    <script type="text/javascript">
        ...
        var url;
        function onAdd(){
            $('#dlg').dialog('open').dialog('setTitle','New DataRow');
            $('#fm').form('clear');
            $('#dataRow_status').combobox('select', '1');
            //$("#dataRow_remark").val('test');
        }
        ...
        function onEdit(){
            ...
            if (row){
            	var entityName='dataRow';
            	var obj = getRowData(entityName, row);
                $('#dlg').dialog('open').dialog('setTitle','Edit DataRow');
                $('#fm').form('clear');
                $('#fm').form('load',obj);
            }
        ...
        function onDestroy(){
            ...
            if (row){
                $.messager.confirm('Confirm','Are you sure you want to destroy this data?',function(r){
                    if (r){
                        $.post('/ext/dataRow/delete',{id:row.cid},function(result){
                            if (result.success){
                            	doSearchReload();    // reload the user data
                            } else {
